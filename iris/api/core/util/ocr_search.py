...
def text_search_by(what, match_case=True, in_region=None, multiple_matches=False):
        ...
        for local_match_index, local_match_object in enumerate(local_text_dict):
            if local_what == local_match_object['value']:
                if local_multiple_matches:
                    ...
                    save_ocr_debug_image(debug_img, [debug_data[local_match_index]])
    ...
    def _search_for_phrase(local_what, local_text_dict, local_multiple_matches):
        ...
        if l_words_len > 0:
            ...
            save_ocr_debug_image(debug_img, temp_debug)
        return return_single

    if not isinstance(what, str):
        return ValueError(INVALID_GENERIC_INPUT)

    text_dict, debug_img, debug_data = text_search_all(True, in_region)
    ...
    logger.debug('> All words on region/screen: ' + ocr_matches_to_string(text_dict))
