...
class Test(BaseTest):
    ...
    def run(self):
        ...
        expected = exists(check_engine_pattern, 10)
        ...
        while region.exists(check_engine_pattern, 2):
            click(check_engine_pattern)

        try:
            expected = wait_vanish(check_engine_pattern, 5)
        ...
        for i in range(pattern_list.__len__()):
            try:
                expected = wait_vanish(pattern_list[i], 10)
