...
class Test(BaseTest):
    ...
    def run(self):
        ...
        expected = exists(search_using_google_pattern, Settings.FIREFOX_TIMEOUT)
        ...
        expected = exists(duckduckgo_search_bar_pattern, Settings.FIREFOX_TIMEOUT)
        assert_true(self, expected, 'Search engine is visible.')

        hover(duckduckgo_search_bar_pattern)

        expected = exists(search_duckduckgo_pattern, Settings.FIREFOX_TIMEOUT)
        ...
        expected = exists(search_duckduckgo_pattern, Settings.FIREFOX_TIMEOUT)
        assert_true(self, expected, 'Pressing tab selects the one-click search engines while the search suggestions are'
                                    ' skipped.')

        # Select ctrl/cmd + arrow UP and arrow DOWN keys.
        change_search_next()

        expected = exists(bing_search_engine_pattern, Settings.FIREFOX_TIMEOUT)
        assert_true(self, expected, 'The default search engine becomes the next one from the list.')

        change_search_previous()

        expected = exists(google_search_engine_pattern, Settings.FIREFOX_TIMEOUT)
