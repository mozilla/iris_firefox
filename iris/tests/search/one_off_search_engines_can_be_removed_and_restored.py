...
class Test(BaseTest):
    ...
    def __init__(self):
        ...
        self.meta = 'Items from the list of one-click search engines can be removed and restored.'
        self.test_case_id = '4276'
    ...
    def run(self):
        ...
        assert_true(self, expected, 'The \'Amazon\' search engine found in the one-offs list.')
        ...
        expected = exists(amazon_search_bar_pattern, 10)
        assert_true(self, expected, 'The \'Amazon\' search engine found in the \'about:preferences#search\' page.')

        click(amazon_search_bar_pattern)

        expected = exists(remove_pattern, 10)
        assert_true(self, expected, 'The \'Remove\' button found.')
        ...
        assert_false(self, expected, 'The search engine previously removed is no longer displayed in the list.')
        ...
        expected = exists(about_preferences_search_page_pattern, 10)
        assert_true(self, expected, 'The \'about:preferences#search\' page loaded.')

        type(Key.TAB)

        expected = exists(restore_default_search_engine_pattern, 10)
        assert_true(self, expected, 'The \'Restore Default Search Engines\' button found.')

        click(restore_default_search_engine_pattern)

        expected = exists(amazon_search_bar_pattern, 10)
        assert_true(self, expected, 'The search engine previously removed is placed back in the list.')
        ...
        expected = exists(amazon_search_bar_pattern, 3)
        assert_true(self, expected, 'The search engine is restored.')
