...
class Test(BaseTest):
    ...
    def __init__(self):
        ...
        self.meta = 'Malicious warning.'
        self.test_case_id = '99498'
    ...
    def run(self):
        ...
        assert_true(self, expected, 'Malicious file has been found.')
        ...
        assert_true(self, expected, 'Blocked download icon is displayed.')

        expected = exists(DownloadManager.DownloadsPanel.ADD_REMOVE_DOWNLOADS_RED_ARROW, 10)
        assert_true(self, expected, 'Open malicious download arrow is displayed.')

        expected = exists(DownloadManager.DownloadsPanel.VIRUS_OR_MALWARE_DOWNLOAD, 10)
        assert_true(self, expected, 'Virus or malware message is displayed.')

        width, height = DownloadManager.DownloadsPanel.ADD_REMOVE_DOWNLOADS_RED_ARROW.get_size()
        mouse_move(DownloadManager.DownloadsPanel.ADD_REMOVE_DOWNLOADS_RED_ARROW.target_offset(width + 10, height / 2))

        expected = exists(DownloadManager.DownloadsPanel.DownloadDetails.BLOCKED_DOWNLOAD, 10)
        assert_true(self, expected, 'Download is highlighted red.')
        ...
        assert_true(self, expected, 'Download details blocked icon is displayed.')
        ...
        if Settings.get_os() == Platform.MAC:
            ...
            assert_true(self, expected, 'Malicious file is displayed.')
        ...
        assert_false(self, expected, 'Malicious file was deleted from Downloads Library.')
        click_window_control('close')
