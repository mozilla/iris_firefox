...
class Test(BaseTest):
    ...
    def run(self):
        ...
        library_menu_opened = exists(other_bookmarks_folder_pattern, DEFAULT_SYSTEM_DELAY)
        ...
        firefox_bookmark_available = exists(firefox_bookmark_pattern, DEFAULT_SYSTEM_DELAY)
        assert_true(self, firefox_bookmark_available,
                    '\'Firefox\' bookmark available in \'Other bookmarks\' folder in \'Library\' menu')

        focus_bookmark_available = exists(focus_bookmark_pattern, DEFAULT_SYSTEM_DELAY)
        ...
        paste('test')
        type(Key.ENTER)
        bookmark_tagged = exists(firefox_bookmark_with_tag_pattern, DEFAULT_SYSTEM_DELAY)
        ...
        paste('test')
        type(Key.ENTER)
        bookmark_tagged = exists(focus_bookmark_with_tag_pattern, DEFAULT_SYSTEM_DELAY)
        ...
        test_tag_displayed = exists(test_tag_pattern, DEFAULT_UI_DELAY_LONG)
        assert_true(self, test_tag_displayed, 'The recent bookmark tags are displayed.')

        right_click(test_tag_pattern)
        open_all_in_tabs_option_available = exists(open_all_in_tabs_option_pattern, DEFAULT_UI_DELAY_LONG)
