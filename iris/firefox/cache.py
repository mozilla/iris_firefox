...
class DiskCache(object):
    ...
    def delete(self, id_or_path=None):
        """Delete cache entries by ID or full path. Delete everything if no ID or path given."""
    ...
    def purge(self, maximum_age=None):
        ...
        for cache_id in self:
            path = self[cache_id]
            m_time = os.path.getmtime(path)  # Modification time as epoch (might have just 1.0s resolution)
            if m_time < stale_limit:
