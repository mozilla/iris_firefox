...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        mozilla_bookmark_history_pattern = (
        ...
        iris_displayed_in_history = right_upper_corner.exists(
            iris_bookmark_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            iris_displayed_in_history is True
        ), "Iris page is displayed in the History menu list."

        # Click on the Show All History button.
        click(show_all_history_pattern, 2)

        today_option_exists = exists(
            history_today_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        mozilla_displayed_in_history = exists(
            mozilla_bookmark_history_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            mozilla_displayed_in_history is True
        ...
        assert (
            iris_page_displayed is True
        ), "Iris local page is still open in the first tab."
