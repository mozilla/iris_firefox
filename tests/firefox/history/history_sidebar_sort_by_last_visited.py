...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        history_sidebar_sort_by_last_visited_pattern = (
            Sidebar.HistorySidebar.ViewBy.VIEW_BY_LAST_VISITED
        )
        history_sidebar_items_sort_by_last_visited_pattern = Pattern(
            "history_sidebar_items_sort_by_last_visited.png"
        )

        # Open some pages to create some history.
        navigate(LocalWeb.MOZILLA_TEST_SITE)

        mozilla_test_site = exists(
        ...
        history_sort_last_visited = exists(
            history_sidebar_sort_by_last_visited_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
        )
        assert (
            history_sort_last_visited is True
        ), "Sort by last visited option is displayed properly."

        click(history_sidebar_sort_by_last_visited_pattern, 1)

        history_sidebar_sort_by_last_visited = exists(
            history_sidebar_items_sort_by_last_visited_pattern,
            FirefoxSettings.SITE_LOAD_TIMEOUT,
        )
        assert (
            history_sidebar_sort_by_last_visited is True
        ), "History list is sorted properly by last visited."
