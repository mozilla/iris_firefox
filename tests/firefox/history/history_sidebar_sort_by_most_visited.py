...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        history_sidebar_sort_by_most_visited_pattern = (
            Sidebar.HistorySidebar.ViewBy.VIEW_BY_MOST_VISITED
        )
        history_sidebar_items_sorted_pattern = Pattern(
            "history_sidebar_items_sort_by_most_visited.png"
        )
        ...
        history_sidebar_sort_by_most_visited = exists(
            history_sidebar_sort_by_most_visited_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
        )
        assert (
            history_sidebar_sort_by_most_visited is True
        ), "Sort by most visited option is displayed properly."

        click(history_sidebar_sort_by_most_visited_pattern, 1)

        history_sidebar_items_sorted = exists(
            history_sidebar_items_sorted_pattern.similar(0.7),
            FirefoxSettings.SITE_LOAD_TIMEOUT,
        )
        assert (
            history_sidebar_items_sorted is True
        ), "History list is sorted properly by most visited."
