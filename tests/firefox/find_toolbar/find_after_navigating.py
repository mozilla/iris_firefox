...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        navigate_load_listener_page_title_pattern = Pattern(
            "navigate_page_title.png"
        ).similar(0.6)
        ...
        page_title_pattern_exists = exists(
            page_title_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        ...
        word_browser_green_exists = exists(
            word_browser_green_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            word_browser_green_exists
        ), 'The items corresponding to "browser" are found.'

        # Click a link (e.g "load listener")
        link_load_listener_exists = exists(
            link_load_listener_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert link_load_listener_exists, 'Link "load listener" found'

        click(link_load_listener_pattern, 1)

        navigate_to_page_loaded = exists(
            navigate_load_listener_page_title_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        phrase_not_found_label_exists = exists(
            FindToolbar.FIND_STATUS_PHRASE_NOT_FOUND,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
        )
        assert (
            phrase_not_found_label_exists
        ), "No visible issue of the highlighted items are present."
