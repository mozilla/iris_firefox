...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        navigated_to_pdf_url = exists(
            pdf_logo_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        ...
        first_occurrence_highlighted = exists(
            first_occurrence_highlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert first_occurrence_highlighted, "The first occurrence is highlighted"

        second_occurrence_unhighlighted = exists(
            second_occurrence_unhighlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            second_occurrence_unhighlighted
        ), "The second occurrence is not highlighted"

        # Navigate through found items
        find_next()

        second_occurrence_highlighted = exists(
            second_occurrence_highlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert second_occurrence_highlighted, "The second occurrence is highlighted"

        # Scroll the page up and down
        find_previous()
        first_occurrence_before_scrolling_highlighted = exists(
            first_occurrence_highlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            first_occurrence_before_scrolling_highlighted
        ), "The first occurrence is highlighted before scrolling"

        repeat_key_down(4)
        repeat_key_up(4)

        first_occurrence_after_scrolling_highlighted = exists(
            first_occurrence_highlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            first_occurrence_after_scrolling_highlighted
