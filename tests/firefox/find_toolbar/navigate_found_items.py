...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        selected_label_exists = exists(
            LocalWeb.SOAP_WIKI_SEE_LABEL, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            selected_label_exists
        ), "The first one has a green background highlighted."

        not_highlighted_label_exists = exists(
            LocalWeb.SOAP_WIKI_SEE_LABEL_UNHIGHLITED, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        cleaning_see_label_exists = exists(
            LocalWeb.SOAP_WIKI_CLEANING_SEE_SELECTED_LABEL,
            FirefoxSettings.FIREFOX_TIMEOUT,
        )
        assert (
            cleaning_see_label_exists
        ), "Navigation using toolbar up arrow works fine."
        ...
        one_of_five_label_exists = exists(
            one_of_five_label_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert one_of_five_label_exists, (
            "The number of the highlighted item changes when "
            'navigating. ("1 of 5" expected)'
        )

        click(find_next_location)

        two_of_five_label_exists = exists(
            two_of_five_label_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert two_of_five_label_exists, (
            "The number of the highlighted item changes when "
