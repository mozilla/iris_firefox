...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        word_mozilla_second_selected_pattern = Pattern(
            "word_mozilla_second_selected.png"
        )
        word_mozilla_second_unselected_pattern = Pattern(
            "word_mozilla_second_unselected.png"
        ).similar(0.7)
        ...
        navigated_to_find_bug = exists(
            word_mozilla_second_unselected_pattern,
        ...
        assert (
            find_toolbar_is_opened
        ), "The Find Toolbar is successfully displayed by pressing CTRL + F / Cmd + F,."

        type("Mozilla")

        selected_label_exists = exists(
            word_mozilla_first_selected_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            text_region,
        )
        ...
        before_next_selected_label_y = find(
            word_mozilla_first_selected_pattern, text_region
        ).y
        find_next()

        after_next_selected_label_y = find(
            word_mozilla_second_selected_pattern, text_region
        ).y
        assert (
            before_next_selected_label_y != after_next_selected_label_y
        ), "Selected label moved forward."

        before_prev_selected_label_y = find(
            word_mozilla_second_selected_pattern, text_region
        ).y
        find_previous()

        after_prev_selected_label_y = find(
            word_mozilla_first_selected_pattern, text_region
        ).y
        assert (
            before_prev_selected_label_y != after_prev_selected_label_y
        ), "Selected label moved backward."

        before_scroll_selected_exists = exists(
            word_mozilla_first_selected_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            text_region,
        )
        ...
        after_scroll_selected_exists = exists(
            word_mozilla_first_selected_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            text_region,
        )
        assert before_scroll_selected_exists and after_scroll_selected_exists, (
            "Occurrence is present after "
