...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        style_text_first_not_selected_pattern = Pattern(
        ...
        navigate(
            vertical_search_page_local
        )  # https://bug1279704.bmoattachments.org/attachment.cgi?id=8762295

        navigated_to_style_url = exists(
            style_text_url_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        first_text_unselected_exists = exists(
            style_text_first_not_selected_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert first_text_unselected_exists, "Style text is unselected before search"

        type("testcase")

        first_text_selected_exists = exists(
            style_text_first_selected_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        second_text_selected_exists = exists(
            style_text_second_selected_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        last_text_selected_exist = exists(
            style_text_last_selected_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
