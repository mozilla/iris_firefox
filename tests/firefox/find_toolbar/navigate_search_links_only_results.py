...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        soap_other_link_highlighted_pattern = Pattern(
            "soap_link_disambiguation_highlighted.png"
        )
        ...
        soap_link_not_highlighted = exists(
            soap_link_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        soap_other_link_highlighted = exists(
            soap_other_link_highlighted_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )

        type(Key.F3, KeyModifier.SHIFT)

        soap_link_highlighted = exists(
            LocalWeb.SOAP_WIKI_SOAP_LINK_HIGHLIGHTED, FirefoxSettings.FIREFOX_TIMEOUT
        )
        soap_other_link_not_highlighted = exists(
            soap_other_link_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            soap_link_not_highlighted
            and soap_other_link_highlighted
            and soap_link_highlighted
            and soap_other_link_not_highlighted
