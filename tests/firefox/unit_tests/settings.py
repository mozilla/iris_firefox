...
class Test(FirefoxTest):
    @pytest.mark.details(description="Unit tests for Settings Class")
    def run(self, firefox):
        ...
        assert (
            Settings.DEFAULT_WAIT_SCAN_RATE == default_wait_scan_rate
        ), "Default wait_scan_rate should be equal to {}".format(
            Settings.DEFAULT_WAIT_SCAN_RATE
        )
        ...
        assert (
            Settings.wait_scan_rate == updated_wait_scan_rate
        ), "Updated value for wait_scan_rate should be equal to {}".format(
            updated_wait_scan_rate
        )
        ...
        assert (
            Settings.DEFAULT_MOVE_MOUSE_DELAY == default_move_mouse_delay
        ), "Default move_mouse_delay should be equal to {}".format(
            Settings.DEFAULT_MOVE_MOUSE_DELAY
        )
        updated_move_mouse_delay = 1
        Settings.move_mouse_delay = updated_move_mouse_delay
        assert (
            Settings.move_mouse_delay == updated_move_mouse_delay
        ), "Updated value for move_mouse_delay should be {}".format(
            updated_move_mouse_delay
        )
        ...
        assert (
            updated_click_delay == Settings.click_delay
        ), "Updated value for click_delay should be equal to {}".format(
            updated_click_delay
        )
        ...
        assert (
            Settings.DEFAULT_MIN_SIMILARITY == default_min_similarity
        ), "Default min_similarity should be equal to {}".format(
            Settings.DEFAULT_MIN_SIMILARITY
        )
        updated_min_similarity = 0.5
        Settings.min_similarity = updated_min_similarity
        assert (
            Settings.min_similarity == updated_min_similarity
        ), "Updated value for min_similarity should be {}".format(
            updated_min_similarity
        )
        ...
        assert (
            Settings.DEFAULT_AUTO_WAIT_TIMEOUT == default_auto_wait_timeout
        ), "Default auto_wait_timeout should be equal to {}".format(
            Settings.DEFAULT_AUTO_WAIT_TIMEOUT
        )
        ...
        assert (
            Settings.auto_wait_timeout == updated_auto_wait_timeout
        ), "Updated value for auto_wait_timeout should be {}".format(
            updated_auto_wait_timeout
        )
        ...
        assert (
            Settings.DEFAULT_SLOW_MOTION_DELAY == default_slow_motion_delay
        ), "Default slow_motion_delay should be equal to {}".format(
            Settings.DEFAULT_SLOW_MOTION_DELAY
        )
        updated_slow_motion_delay = 3
        Settings.slow_motion_delay = updated_slow_motion_delay
        assert (
            Settings.slow_motion_delay == updated_slow_motion_delay
        ), "Updated value for slow_motion_delay should be {}".format(
            updated_slow_motion_delay
        )
        ...
        assert (
            Settings.DEFAULT_OBSERVE_SCAN_RATE == default_observe_scan_rate
        ), "Default observe_scan_rate should be equal to {}".format(
            Settings.DEFAULT_OBSERVE_SCAN_RATE
        )
        updated_observe_scan_rate = 5
        Settings.observe_scan_rate = updated_observe_scan_rate
        assert (
            Settings.observe_scan_rate == updated_observe_scan_rate
        ), "Updated value for observe_scan_rate should be {}".format(
            updated_observe_scan_rate
        )
        ...
        assert (
            Settings.observe_min_changed_pixels == updated_observe_min_changed_pixels
        ), "Updated value for observe_min_changed_pixels should be {}".format(
