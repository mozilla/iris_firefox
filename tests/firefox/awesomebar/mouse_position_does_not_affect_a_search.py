...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        top_two_third_screen_region = Region(
        ...
        search_engine_logo_icon = exists(
            duck_duck_go_one_off_button.similar(0.7),
        ...
        search_results_listed = exists(
            search_results_listed_pattern.similar(0.7), FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        Mouse().move(
            Location(search_engine_location.x + 1, search_engine_location.y + 1),
            duration=0,
        )
        ...
        result_is_focused = exists(
            result_is_focused_pattern.similar(0.7), FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        firefox_local_page_loaded = exists(
            LocalWeb.FIREFOX_LOGO, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        iris_local_page_loaded = exists(
            LocalWeb.IRIS_LOGO, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )

        assert firefox_local_page_loaded or iris_local_page_loaded, (
            'Page successfully is loaded. The browser does NOT search for "bug" with'
            + " the search engine that had the mouse over it."
        )
