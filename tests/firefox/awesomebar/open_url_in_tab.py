...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert (
            expected
        ), "The 'browser.search.openintab' preference has status 'default' by default."

        expected = region.exists(false_value_no_highlight_pattern, 10)
        assert (
            expected
        ), "The 'browser.search.openintab' preference has value 'false' by default."

        double_click(default_status_pattern)

        expected = region.exists(modified_status_pattern, 10)
        assert expected, (
            "The 'browser.search.openintab' preference has status 'modified' after the "
            "preference has changed."
        )

        expected = region.exists(true_value_highlight_pattern, 10)
        assert expected, (
            "The 'browser.search.openintab' preference has value 'true' after the "
            "preference has changed."
        )
