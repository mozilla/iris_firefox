...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert current_version in FirefoxUtils.get_firefox_version(
        ...
        if is_update_required(current_version, starting_condition):
            for update_step in update_steps_list:
                ...
                assert update_step in FirefoxUtils.get_firefox_version(
                    firefox.application.path
        ...
        if OSHelper.is_windows() or OSHelper.is_linux():
            try:
                expected = wait_vanish(google_one_off_button_pattern, 10)
                assert expected, (
                    "The removed search engine from the one-off searches bar in previous Firefox "
                    + "version is still removed in the latest Firefox version."
                )
            except FindError:
                raise FindError(
                    "The removed search engine from the one-off searches bar in previous Firefox version "
        ...
        expected = exists(startpage_https_search_engine_pattern, 10)
        assert expected, (
            "The new search engine added in previous Firefox version found in the "
