...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        expected = exists(AboutPreferences.ABOUT_PREFERENCE_SEARCH_PAGE_PATTERN, FirefoxSettings.FIREFOX_TIMEOUT)
        assert expected is True, "The 'about:preferences#search' page successfully loaded."

        expected = exists(AboutPreferences.FIND_IN_OPTIONS, FirefoxSettings.FIREFOX_TIMEOUT)
        assert expected is True, "'Find in Options' search field is displayed."

        click(AboutPreferences.FIND_IN_OPTIONS)
        paste("dangerous")

        expected = exists(AboutPreferences.DECEPTIVE_CONTENT_AND_DANGEROUS_SOFTWARE, FirefoxSettings.FIREFOX_TIMEOUT)
        assert expected is True, "Deceptive content and dangerous software section is displayed."

        expected = exists(AboutPreferences.BLOCK_DANGEROUS_DOWNLOADS, FirefoxSettings.FIREFOX_TIMEOUT)
        assert expected is True, "'Block dangerous downloads' option is available."
        expected = exists(AboutPreferences.WARN_UNWANTED_UNCOMMON_SOFTWARE, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        if region_option_1.exists(AboutPreferences.CHECKED_BOX, FirefoxSettings.FIREFOX_TIMEOUT):
            click(AboutPreferences.BLOCK_DANGEROUS_AND_DECEPTIVE_CONTENT)

        expected = region_option_1.exists(AboutPreferences.UNCHECKED_BOX, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        expected = exists(DownloadFiles.DOWNLOADS_PANEL_CONTENT1_COMPLETED, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        assert expected is True, "Download button found in the page."
        click(NavBar.DOWNLOADS_BUTTON)
        expected = exists(DownloadFiles.DOWNLOADS_PANEL_BADREP_COMPLETED.similar(0.7), FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        assert expected is True, "Download button found in the page."
        click(NavBar.DOWNLOADS_BUTTON)
        expected = exists(DownloadFiles.DOWNLOADS_PANEL_UNKNOWN_COMPLETED.similar(0.75),
        ...
        assert expected is True, "Download button found in the page."
        click(NavBar.DOWNLOADS_BUTTON)
        expected = exists(DownloadFiles.DOWNLOADS_PANEL_UNKNOWN1_COMPLETED, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        assert expected is True, "Download button found in the page."
        click(NavBar.DOWNLOADS_BUTTON)
        expected = exists(DownloadFiles.DOWNLOADS_PANEL_PUA_COMPLETED.similar(0.75), FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        click(AboutPreferences.BLOCK_DANGEROUS_AND_DECEPTIVE_CONTENT)
        expected = region_option_1.exists(AboutPreferences.CHECKED_BOX, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        expected = region_option_2.exists(AboutPreferences.UNCHECKED_BOX, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        expected = region_file.exists(DownloadManager.DownloadsPanel.BLOCKED_DOWNLOAD_ICON,
                                      FirefoxSettings.FIREFOX_TIMEOUT)
