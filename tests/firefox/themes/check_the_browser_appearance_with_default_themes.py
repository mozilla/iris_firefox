...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert True is not expected, 'ENABLE button NOT found in the page.'

        previous_tab()

        expected = exists(AboutAddons.Themes.IRIS_TAB_LIGHT_OR_DEFAULT_THEME, 10)
        assert True is expected, 'Tab information is correctly displayed for DEFAULT theme.'

        open_library()

        expected = exists(Library.BOOKMARKS_MENU, 10)
        assert True is expected, 'Expand bookmarks menu button displayed properly.'
        ...
        expected = exists(AboutAddons.Themes.IRIS_TAB_LIGHT_OR_DEFAULT_THEME, 10)
        assert True is expected, 'LIGHT theme successfully applied.'
