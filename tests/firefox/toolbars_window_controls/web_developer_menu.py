...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        left_corner_region = Region(
            0,
            Screen.SCREEN_HEIGHT / 2,
            Screen.SCREEN_WIDTH / 2,
            Screen.SCREEN_HEIGHT / 2,
        )
        right_corner_region = Region(
            Screen.SCREEN_WIDTH / 2,
            Screen.SCREEN_HEIGHT / 2,
            Screen.SCREEN_WIDTH / 2,
            Screen.SCREEN_HEIGHT / 2,
        ...
        web_developer_text_assert = Screen().exists(
            Pattern("web_developer_insert.png"), 10
        )
        ...
        if OSHelper.is_mac():
            try:
                close_button_assert = left_corner_region.wait_vanish(
                    Pattern("web_developer_close_button.png"), 5
                )
            ...
            resize_region = Region(
                left_corner_region.x,
                left_corner_region.y - 100,
                left_corner_region.width,
                left_corner_region.height,
            )
            ...
            try:
                dev_close_button_assert = right_corner_region.wait_vanish(
                    Pattern("web_developer_close_button.png"), 4
                )
            ...
            for word in console_items:
                assert left_corner_region.exists(word), "Item is present: '{}'".format(
                    word
                )
