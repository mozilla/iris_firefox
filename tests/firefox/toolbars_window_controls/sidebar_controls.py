...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert exists(
            sidebar_title_pattern, 10
        ), "Sidebar title was displayed properly."

        region = Region(
            0,
            find(sidebar_title_pattern).y,
            Screen.SCREEN_WIDTH / 4,
            Screen.SCREEN_HEIGHT / 4,
        )
        assert region.exists(
            x_button_sidebar_pattern, 10
        ), "Close button was displayed properly."

        region.hover(x_button_sidebar_pattern)
        assert region.exists(
            x_button_sidebar_hovered_pattern, 10
        ), "Hover state displayed properly."

        region.click(x_button_sidebar_hovered_pattern)
        try:
            assert wait_vanish(
                sidebar_title_pattern, 10
            ), "Sidebar was closed successfully."
        except FindError:
