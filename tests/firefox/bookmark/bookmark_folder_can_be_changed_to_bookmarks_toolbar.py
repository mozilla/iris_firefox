...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        bookmark_toolbar_folder_displayed = exists(
            Bookmarks.StarDialog.PANEL_OPTION_BOOKMARK_TOOLBAR.similar(0.6), region=stardialog_region
        )
        ...
        test_bookmark_folder_changed = exists(
            LocalWeb.FOCUS_BOOKMARK.similar(0.6), FirefoxSettings.FIREFOX_TIMEOUT * 3, bookmarks_toolbar_region
        )
        assert test_bookmark_folder_changed is True, (
            "The Bookmarks Toolbar is enabled and the saved bookmark " "is correctly displayed."
        )
