...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        bookmarks_menu_added_to_toolbar_pattern = Pattern('bookmarks_menu_added_to_toolbar.png')

        library_button_exists = exists(library_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT/2)
        assert library_button_exists is True, 'View history, saved bookmarks and more section exists'

        click(library_button_pattern)

        bookmarks_menu_option_exists = exists(bookmarks_menu_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT/2)
        assert bookmarks_menu_option_exists is True, 'Bookmarks menu option exists'

        click(bookmarks_menu_option_pattern)

        bookmarking_tools_exists = exists(bookmarking_tools_pattern, FirefoxSettings.FIREFOX_TIMEOUT/2)
        ...
        view_bookmarks_sidebar_exists = exists(view_bookmarks_sidebar_pattern, FirefoxSettings.FIREFOX_TIMEOUT/2)
        assert view_bookmarks_sidebar_exists is True, 'View Bookmarks Sidebar is displayed'

        view_bookmarks_toolbar_exists = exists(view_bookmarks_toolbar_pattern, FirefoxSettings.FIREFOX_TIMEOUT/2)
        ...
        bookmarks_menu_added_to_toolbar_exists = exists(bookmarks_menu_added_to_toolbar_pattern,
                                                        FirefoxSettings.SHORT_FIREFOX_TIMEOUT)
        assert bookmarks_menu_added_to_toolbar_exists is True, 'The Bookmarks menu is correctly added ' \
