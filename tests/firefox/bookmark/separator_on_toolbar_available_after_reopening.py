...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        most_visited_toolbar_bookmarks_folder_pattern = Pattern(
            "most_visited_bookmarks.png"
        )
        ...
        location_of_most_visited_folder = find(
            most_visited_toolbar_bookmarks_folder_pattern
        )
        region_with_separator = Region(
            location_of_most_visited_folder.x - 5,
            location_of_most_visited_folder.y - 5,
            50,
            50,
        )
        ...
        assert new_separator_option_available is True, (
            "'New separator' option available in context menu after"
            " right click at item in Bookmarks Toolbar"
        )

        click(new_separator_option_pattern)

        separator_appeared = exists(
            toolbar_separator_pattern, region=region_with_separator
        )
        assert separator_appeared is True, (
            "A separator is placed in front of the first bookmark/folder "
        ...
        try:
            toolbar_disabled = wait_vanish(
                most_visited_toolbar_bookmarks_folder_pattern
            )
            assert (
                toolbar_disabled is True
            ), "The Bookmark Toolbar is no longer displayed."
        except FindError:
            raise FindError("Toolbar did not close")

        open_bookmarks_toolbar()

        bookmarks_displayed_properly = exists(
            toolbar_bookmarks_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert bookmarks_displayed_properly is True, (
            "Toolbar opened. All the bookmarks/folders are in the proper "
        ...
        assert (
            separator_appeared is True
        ), "Separator available on toolbar after reopening"
