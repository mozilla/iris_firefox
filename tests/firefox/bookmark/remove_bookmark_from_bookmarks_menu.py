...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        try:
            ...
            raise FindError(
                "Moz bookmark is not present in the Bookmarks Menu, aborting."
            )

        click(delete_pattern)

        try:
            delete_bookmark_menu_assert = right_upper_corner.wait_vanish(
                menu_bookmark_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
            assert delete_bookmark_menu_assert is True, (
