...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        tabs = [
            google_logo_pattern,
            LocalWeb.IRIS_LOGO_ACTIVE_TAB,
            LocalWeb.POCKET_LOGO,
            LocalWeb.MOZILLA_LOGO,
            LocalWeb.FOCUS_LOGO,
            LocalWeb.FIREFOX_LOGO,
            LocalWeb.IRIS_LOGO_ACTIVE_TAB,
            el_pais_logo_pattern,
            el_pais_logo_pattern,
            yahoo_logo_pattern,
        ]
        ...
        open_all_in_tabs_option_exists = exists(
            open_all_in_tabs_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        for tab in tabs:
            ...
            assert site_from_list_opened is True, (
                "The website #" + str(num) + " from 'Most Visited' section " "is opened"
            )
            next_tab()

        all_websites_are_opened = exists(LocalWeb.IRIS_LOGO)
        assert all_websites_are_opened is True, (
            "All the websites from the Most visited section are opened in "
