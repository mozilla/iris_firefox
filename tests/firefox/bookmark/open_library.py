...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert (
            library_opened_vis_shortcut is True
        ), "Library opened via CMD/CTRL+Shift+B"

        other_bookmarks_folder_displayed = exists(Library.OTHER_BOOKMARKS)
        assert (
            other_bookmarks_folder_displayed is True
        ), "Other Bookmarks folder displayed"

        click(Library.OTHER_BOOKMARKS)

        other_bookmarks_is_default_shortcut = exists(wiki_bookmark_logo_pattern)
        assert (
            other_bookmarks_is_default_shortcut is True
        ), "Other Bookmarks is set as default"
        ...
        assert (
            library_opened_from_menu is True
        ), "Library opened from View History, saved bookmarks and more"
        ...
        if OSHelper.is_linux() or OSHelper.is_mac():
            ff_menu_show_all_bookmarks_exists = exists(
                ff_menu_show_all_bookmarks_pattern
            )
            assert (
                ff_menu_show_all_bookmarks_exists is True
            ), "Firefox menu > Bookmarks > Show All Bookmarks exists"

            click(ff_menu_show_all_bookmarks_pattern)
        else:
            ff_menu_show_all_bookmarks_exists = exists(
                show_all_bookmarks_button_pattern
            )
            assert (
                ff_menu_show_all_bookmarks_exists is True
            ), "Firefox menu > Bookmarks > Show All Bookmarks exists"
        ...
        assert (
            library_opened_from_ff_menu is True
        ), "Library opened from View History, saved bookmarks and more"
