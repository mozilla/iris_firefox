...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        new_bookmark_window_exists = exists(
            Bookmarks.StarDialog.NAME_FIELD, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert new_bookmark_window_exists is True, "New Bookmark window is displayed"

        paste("Focus")

        folders_expander_exists = exists(
            Bookmarks.StarDialog.PANEL_FOLDERS_EXPANDER.similar(0.6),
        ...
        bookmarks_toolbar_option_exists = exists(
            Library.BOOKMARKS_TOOLBAR, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            bookmarks_toolbar_option_exists is True
        ), "Bookmark toolbar folder option is displayed"

        click(Library.BOOKMARKS_TOOLBAR)

        tags_field_exists = exists(
        ...
        bookmark_added_to_toolbar = exists(
            LocalWeb.FOCUS_BOOKMARK_SMALL, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            bookmark_added_to_toolbar is True
        ), "The bookmark is correctly added to Bookmarks Toolbar."
