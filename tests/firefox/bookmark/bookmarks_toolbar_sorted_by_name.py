...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        try:
            ...
            raise FindError(
                "Moz bookmark is NOT present inside the Bookmarks Sidebar, aborting."
            )

        moz_bookmark_drag_assert = exists(
            dragged_bookmark_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            moz_bookmark_drag_assert is True
        ), "Moz Bookmark was dragged successfully."
        ...
        try:
            ...
            raise FindError(
                "Firefox bookmark is NOT present inside the Bookmarks Sidebar, aborting."
            )

        firefox_bookmark_drag_assert = exists(
            firefox_toolbar_bookmark, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            firefox_bookmark_drag_assert is True
        ), "Firefox Bookmark was dragged successfully."
        ...
        try:
            ...
            raise FindError(
                "Pocket bookmark is NOT present inside the Bookmarks Sidebar, aborting."
            )

        pocket_bookmark_drag_assert = exists(
            pocket_toolbar_bookmark, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            pocket_bookmark_drag_assert is True
        ), "Pocket Bookmark was dragged successfully."
        ...
        toolbar_bookmarks_sorted_by_name_assert = exists(
            toolbar_bookmarks_sorted_by_name, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert toolbar_bookmarks_sorted_by_name_assert is True, (
            "The items from the Bookmarks Toolbar has been "
