...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert new_bookmark_option_available is True, (
            "'New Bookmark' option available in context "
            "menu after right click at toolbar."
        )

        click(new_bookmark_option_pattern)

        new_bookmark_popup_opened = exists(
            new_bookmark_window_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            new_bookmark_popup_opened is True
        ), "The 'New Bookmark' popup window is displayed."

        close_window_control("auxiliary")

        try:
            popup_opened = wait_vanish(
                new_bookmark_window_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
            )
        ...
        try:
            ...
            assert new_bookmark_not_created is True, (
                "New Bookmark is not saved as bookmarked inside"
        ...
        assert delete_option_available is True, (
            "'Delete' option available in context menu after"
        ...
        assert getting_started_bookmark_available is False, (
            "Delete bookmark action is successfully performed. "
