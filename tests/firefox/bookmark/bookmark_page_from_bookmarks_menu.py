...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        ), "'View history, saved bookmarks and more' button exists"
        ...
        assert (
            star_button_unstarred_exists is True
        ), "Unstarred star-shaped button exists"

        bookmark_menu_is_displayed = exists(bookmark_this_page_pattern)
        assert (
            bookmark_menu_is_displayed is True
        ...
        assert (
            star_button_starred_exists is True
        ), "Star-shaped button changed its color to blue"
        ...
        assert (
            star_panel_exists is True
        ), "Page Bookmarked menu is displayed under the star-shaped button."

        click(LocationBar.STAR_BUTTON_STARRED)

        try:
            star_panel_not_exists = wait_vanish(
                Bookmarks.StarDialog.NEW_BOOKMARK, FirefoxSettings.FIREFOX_TIMEOUT
            )
            assert star_panel_not_exists is True, (
                "Page Bookmarked menu is auto dismissed if no action is taken "
