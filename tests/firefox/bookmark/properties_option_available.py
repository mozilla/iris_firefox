...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert new_folder_option_available is True, (
            "'New Folder' option available in context menu after "
            "right click at toolbar."
        )

        click(new_folder_option_pattern)

        new_folder_panel_opened = exists(
            new_folder_panel_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert new_folder_panel_opened is True, (
            "'New folder' panel opened after clicking "
            "at the 'New Folder' option from context menu"
        )

        type(Key.ENTER)

        new_folder_created = exists(
            toolbar_new_folder_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            new_folder_created is True
        ), "The folder is created and displayed on the Bookmark Toolbar."
        ...
        assert properties_option_available is True, (
            "'Properties' option available in context menu after right click"
        ...
        assert folder_properties_opened is True, (
            "The Properties option is active and clickable. Note: In the "
