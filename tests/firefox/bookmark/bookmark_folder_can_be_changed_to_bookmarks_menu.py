...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        test_site_opened = exists(
            LocalWeb.FOCUS_LOGO, FirefoxSettings.FIREFOX_TIMEOUT * 3
        )
        assert test_site_opened is True, "Previously bookmarked Focus website is opened"

        stardialog_region = Rectangle(
        ...
        panel_folder_default_option_exists = exists(
            Bookmarks.StarDialog.PANEL_FOLDER_DEFAULT_OPTION.similar(0.6),
            region=stardialog_region,
        )
        assert (
            panel_folder_default_option_exists is True
        ), "Panel folder default option exists"
        ...
        bookmark_menu_folder_displayed = exists(
            Bookmarks.StarDialog.PANEL_OPTION_BOOKMARK_MENU.similar(0.6),
            region=stardialog_region,
        )
        ...
        stardialog_done_button_displayed = exists(
            Bookmarks.StarDialog.DONE, region=stardialog_region
        )
        assert (
            stardialog_done_button_displayed is True
        ), "Stardialog Done button is displayed"
        ...
        bookmarks_sidebar_location = find(
            Sidebar.BookmarksSidebar.SIDEBAR_BOOKMARKS_TITLE
        )
        bookmarks_width, bookmarks_height = (
            Sidebar.BookmarksSidebar.SIDEBAR_BOOKMARKS_TITLE.get_size()
        )
        bookmarks_sidebar_region = Rectangle(
            0,
        ...
        bookmarks_menu_folder_exists = exists(
            SidebarBookmarks.BOOKMARKS_MENU,
            FirefoxSettings.FIREFOX_TIMEOUT,
        ...
        bookmark_moved = exists(
            LocalWeb.FOCUS_BOOKMARK_SMALL,
            FirefoxSettings.FIREFOX_TIMEOUT,
            bookmarks_sidebar_region,
        )
        assert bookmark_moved is True, (
            "The pop up is dismissed and the bookmark is correctly saved in "
            "Bookmarks menu."
        )
