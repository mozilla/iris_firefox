...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        panel_folder_default_option_exists = exists(
            Bookmarks.StarDialog.PANEL_FOLDER_DEFAULT_OPTION.similar(0.6), region=stardialog_region
        )
        ...
        bookmark_menu_folder_displayed = exists(
            Bookmarks.StarDialog.PANEL_OPTION_BOOKMARK_MENU.similar(0.6), region=stardialog_region
        )
        ...
        bookmarks_sidebar_region = Rectangle(
            0, bookmarks_sidebar_location.y, bookmarks_width * 2, Screen.SCREEN_HEIGHT / 2
        )

        bookmarks_menu_folder_exists = exists(
            SidebarBookmarks.BOOKMARKS_MENU, FirefoxSettings.FIREFOX_TIMEOUT, bookmarks_sidebar_region
        )
        ...
        bookmark_moved = exists(
            LocalWeb.FOCUS_BOOKMARK_SMALL, FirefoxSettings.FIREFOX_TIMEOUT, bookmarks_sidebar_region
        )
        assert bookmark_moved is True, (
            "The pop up is dismissed and the bookmark is correctly saved in " "Bookmarks menu."
        )
