...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        other_bookmarks_folder_assert = exists(
            other_bookmarks_folder_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            other_bookmarks_folder_assert is True
        ), "Other bookmarks folder option present."

        click(other_bookmarks_folder_pattern)

        choose_option_assert = exists(
            choose_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        bookmarks_toolbar_option_assert = exists(
            bookmarks_toolbar_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert bookmarks_toolbar_option_assert, "Bookmarks toolbar option present."

        click(bookmarks_toolbar_option_pattern)

        new_folder_button_assert = exists(
            new_folder_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert new_folder_button_assert is True, "New folder button present."

        click(new_folder_button_pattern)

        new_folder_created_assert = exists(
            new_folder_created_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert new_folder_created_assert is True, "New folder created."

        paste("moz_bookmark")

        done_button_assert = exists(
            done_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert done_button_assert is True, "Done button present."

        click(done_button_pattern)

        bookmark_folder_assert = exists(
            bookmark_folder_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
