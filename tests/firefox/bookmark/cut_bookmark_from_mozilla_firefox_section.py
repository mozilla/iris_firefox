...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        open_all_in_tabs_exists = exists(
            open_all_in_tabs_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            open_all_in_tabs_exists is True
        ), "Bookmarks from Other bookmarks folder displayed"

        bookmark_deleted = exists(mozilla_about_us_bookmark_pattern)
        assert (
            bookmark_deleted is False
        ), "Bookmark is correctly deleted from previous section"
