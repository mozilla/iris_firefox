...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        library_import_bookmarks_from_html_pattern = (
            Library.ImportAndBackup.IMPORT_BOOKMARKS_FROM_HTML
        )
        ...
        import_bookmarks_from_html_dropdown_pattern = Pattern(
            "import_bookmarks_from_html_dropdown.png"
        )
        ...
        library_popup_open = exists(
            library_import_backup_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert library_popup_open is True, "Library is correctly opened."

        click(library_import_backup_pattern)

        import_button_available = exists(
            library_import_bookmarks_from_html_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            import_button_available is True
        ), '"Import bookmarks from HTML" button available'
        ...
        if OSHelper.is_mac():
            ...
            open_bookmark_file = exists(
                import_bookmarks_from_html_dropdown_pattern,
            ...
            import_bookmarks_from_html_dropdown = exists(
                import_bookmarks_from_html_dropdown_pattern,
                FirefoxSettings.FIREFOX_TIMEOUT,
            )
            assert (
                import_bookmarks_from_html_dropdown is True
        ...
        sidebar_bookmarks_button = exists(
            sidebar_bookmarks_menu_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert sidebar_bookmarks_button is True, "Sidebar bookmarks button available"

        click(sidebar_bookmarks_menu_pattern)

        test_bookmark_imported = exists(
            test_bookmark_imported_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            test_bookmark_imported is True
