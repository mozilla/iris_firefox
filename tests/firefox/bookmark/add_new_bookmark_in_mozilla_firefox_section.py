...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        mozilla_firefox_bookmarks_folder_exists = exists(
            mozilla_firefox_bookmarks_folder_pattern,
        ...
        assert (
            mozilla_get_involved_bookmark_exists
        ...
        context_menu_new_bookmark_pattern_displayed = exists(
            Library.Organize.NEW_BOOKMARK, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            context_menu_new_bookmark_pattern_displayed is True
        ), "Context menu New Bookmark option is displayed"

        click(Library.Organize.NEW_BOOKMARK)

        new_bookmark_window_opened = exists(
            new_bookmark_window_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        firefox_menu_bookmarks__second_exists = exists(
            firefox_menu_bookmarks_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            firefox_menu_bookmarks__second_exists is True
        ), "Firefox menu > Bookmarks exists"

        click(firefox_menu_bookmarks_pattern)

        mozilla_firefox_bookmarks_folder_second_exists = exists(
            mozilla_firefox_bookmarks_folder_pattern,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
        )
        assert mozilla_firefox_bookmarks_folder_second_exists is True, (
            "Firefox menu > Bookmarks > Mozilla Firefox " "bookmarks folder exists"
        )
        click(mozilla_firefox_bookmarks_folder_pattern)

        bookmark_exists = exists(
            new_soap_bookmark_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            bookmark_exists is True
        ), "A new bookmark is added in Mozilla Firefox section."
