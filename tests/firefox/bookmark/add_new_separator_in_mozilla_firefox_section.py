...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert (
            firefox_menu_bookmarks_exists is True
        ), "Firefox menu > Bookmarks button exists"
        ...
        new_separator_option_exists = exists(
            Library.Organize.NEW_SEPARATOR, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert new_separator_option_exists is True, "New Folder option exists"

        click(Library.Organize.NEW_SEPARATOR)

        open_all_in_tabs_exists = exists(
            open_all_in_tabs_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert open_all_in_tabs_exists is True, "Open all in tabs option exists"
        ...
        separator_added = exists(
            separator_added_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            separator_added is True
        ), "A new Separator is added in Mozilla Firefox section."
