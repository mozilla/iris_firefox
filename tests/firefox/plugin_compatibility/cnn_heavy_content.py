...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        cnn_weather_page_loaded = exists(
            cnn_weather_page_tab_pattern,
            FirefoxSettings.HEAVY_SITE_LOAD_TIMEOUT,
            tabs_region,
        )
        assert (
            cnn_weather_page_loaded is True
        ...
        first_video_centred = scroll_until_pattern_found(
            north_text_mark_pattern, type, (Key.DOWN,), 20
        )
        ...
        another_video_exists = scroll_until_pattern_found(
            related_video_pattern, page_down, (None,), 20
        )
        assert another_video_exists is True, "Another video is displayed"

        click(related_video_pattern)

        related_video_playing = exists(
            speaker_icon_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT, tabs_region
        )
        assert (
            related_video_playing is True
        ), "The video is playing and there is no browser crashes"
