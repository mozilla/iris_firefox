...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        this_isnt_a_deceptive_site_button_pattern = Pattern(
            "this_isnt_a_deceptive_site_button.png"
        )
        ignore_the_risk_link_pattern = Pattern("ignore_the_risk_link.png")

        navigate("http://www.itisatrap.org/firefox/its-a-trap.html")

        not_blocked_by_google_page_displayed = exists(
            not_blocked_by_google_page_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert (
            not_blocked_by_google_page_displayed
        ), "The page that is not blocked by google opened"
        ...
        ignore_the_risk_link_displayed = exists(
            ignore_the_risk_link_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        assert (
            ...
            + "problem or ignore the risk and go to this unsafe site."
            + "Learn more about deceptive sites and phishing "
            + "at www.antiphishing.org. Learn more about Firefox’s Phishing and Malware Protection at "
            + 'support.mozilla.org."'
        )

        location_to_deselection = Location(
        ...
        itisatrap_page_loaded = exists(
            get_me_out_of_here_button_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert itisatrap_page_loaded, (
            'itisatrap.org page loaded and the button "Get me out of here!" in the banner '
            "at the top displayed"
        )

        this_isnt_a_deceptive_site_button_hidden = exists(
            this_isnt_a_deceptive_site_button_pattern
        )
        assert (
            this_isnt_a_deceptive_site_button_hidden is False
        ), '"This isn\'t a Deceptive site" button is hidden'
