...
class Test(BaseTest):
    ...
    def run(self, firefox):
        ...
        test_page_opened = exists(
            drop_image_data_radiobutton_pattern, Settings.DEFAULT_SITE_LOAD_TIMEOUT
        )
        ...
        paste(
            f"window.resizeTo({Screen.SCREEN_WIDTH * 11//20}, {Screen.SCREEN_HEIGHT * 9 // 10})"
        )
        type(Key.ENTER)
        close_tab()
        ...
        new_window_drop_location = Location(
            Screen.SCREEN_WIDTH * 3 // 5, Screen.SCREEN_HEIGHT // 20
        )
