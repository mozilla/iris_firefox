...
class Test(BaseTest):
    ...
    def run(self, firefox):
        drop_image_data_radiobutton_selected_pattern = Pattern(
            "drop_image_data_selected.png"
        )
        ...
        test_page_opened = exists(
            drop_image_data_radiobutton_pattern, Settings.site_load_timeout
        )
        ...
        assert (
            drop_image_data_selected
        ), 'The "drop-image-data" changed color to red which indicates that it has been selected.'

        drop_result_message_displayed = scroll_until_pattern_found(
            not_matching_message_pattern, type, (Key.DOWN,)
        )
        ...
        paste(
            "window.resizeTo({0}, {1})".format(
                Screen.SCREEN_WIDTH * 9 // 20, Screen.SCREEN_HEIGHT * 9 // 10
            )
        )
        type(Key.ENTER)
        close_tab()
        ...
        opened_tab_location = find(Tabs.NEW_TAB_HIGHLIGHTED).right(
            Screen.SCREEN_WIDTH // 4
        )
        new_window_drop_location = Location(
            Screen.SCREEN_WIDTH * 11 // 20, Screen.SCREEN_HEIGHT // 20
        )

        drag_drop(opened_tab_location, new_window_drop_location)

        drop_position_visible = exists(
            drop_here_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert drop_position_visible, "Drop position can be reached."

        navigate(LocalWeb.SOAP_WIKI_TEST_SITE)
        page_loaded = exists(
            image_from_page_pattern, Settings.DEFAULT_SITE_LOAD_TIMEOUT
        )
        assert (
            page_loaded
        ...
        not_matching_message_appears = exists(
            not_matching_message_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        assert (
            link_displayed
        ), 'Wiki page contains link that will be dropped into "Drop stuff here" area'
        ...
        assert not_matching_message_appears, (
            '"Not Matching" appears under the Drop Stuff Here '
