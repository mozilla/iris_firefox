...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        hamburger_menu_zoom_indicator_pattern = (
        ...
        if (
            OSHelper.is_windows()
        ):  # fix issue when on hover on navbar image is not being recognized
            search_bar_hover_pattern = Pattern("navigation_bar_hover.png")
            expected = expected or exists(
                search_bar_hover_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
        ...
        zoom_increase_button = exists(
            zoom_control_toolbar_increase_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        expected = exists(
            "110%", FirefoxSettings.FIREFOX_TIMEOUT, region=new_region_110
        )
        assert (
            expected
        ...
        assert (
            expected
        ), "Zoom indicator is successfully reset to default in hamburger menu."

        reg_url = create_region_for_url_bar()

        if OSHelper.is_windows() or OSHelper.is_linux():
            expected = exists(
                url_bar_default_zoom_level_pattern.similar(0.95),
                FirefoxSettings.FIREFOX_TIMEOUT,
                region=reg_url,
            )
            ...
            expected = exists(
                url_bar_default_zoom_level_pattern.similar(0.93),
                FirefoxSettings.FIREFOX_TIMEOUT,
                region=reg_url,
            )
            assert expected, "Zoom indicator is displayed anymore in the url bar."
