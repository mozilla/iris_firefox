...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        expected = exists(
            hamburger_menu_zoom_indicator_pattern, FirefoxSettings.FIREFOX_TIMEOUT, region=new_region_default
        )
        ...
        if OSHelper.is_windows() or OSHelper.is_linux():
            expected = exists(
                url_bar_default_zoom_level_pattern.similar(0.95), FirefoxSettings.FIREFOX_TIMEOUT, region=reg_url
            )
            assert expected, "Zoom indicator is displayed anymore in the url bar."
        else:
            expected = exists(
                url_bar_default_zoom_level_pattern.similar(0.93), FirefoxSettings.FIREFOX_TIMEOUT, region=reg_url
            )
            assert expected, "Zoom indicator is displayed anymore in the url bar."
