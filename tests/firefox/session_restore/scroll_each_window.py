...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        proper_hamburger_menu_region = Region(
            0,
            iris_tab_logo_location.y,
        ...
        firefox_tab_is_active = exists(
            firefox_test_site_tab_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        firefox_tab_region_before = Region(
            firefox_tab_location_before.x - tab_width,
            0,
            width=tab_width * 3,
            height=Screen.SCREEN_HEIGHT,
        )
        firefox_tab_region_after = Region(
            (Screen.SCREEN_WIDTH / 2) - tab_width,
            0,
            width=tab_width * 3,
            height=Screen.SCREEN_HEIGHT,
        )
        ...
        focus_test_site_tab_exists = exists(
            focus_test_site_tab_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert focus_test_site_tab_exists, "Focus site tab is active."

        focus_tab_location_before = find(focus_test_site_tab_pattern)
        focus_tab_region_before = Region(
            0,
            focus_tab_location_before.y - (tab_height / 2),
            width=Screen.SCREEN_WIDTH,
            height=tab_height * 2,
        )
        focus_tab_region_after = Region(
            0,
            (Screen.SCREEN_HEIGHT / 2) - tab_height * 5,
            width=Screen.SCREEN_WIDTH,
        ...
        try:
            focus_tab_dragged = wait_vanish(
                focus_test_site_tab_pattern,
        ...
        focus_tab_scrolled = exists(
            focus_tab_scrolled_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert focus_tab_scrolled, "Focus tab scrolled successful."

        # Drag-n-drop Firefox tab
        firefox_tab_drop_location = Location(
            x=Screen.SCREEN_WIDTH / 2, y=tab_height * 4
        )
        drag_drop(firefox_tab_location_before, firefox_tab_drop_location)

        try:
            firefox_test_site_tab_dragged = wait_vanish(
                firefox_test_site_tab_pattern,
                FirefoxSettings.FIREFOX_TIMEOUT,
        ...
        firefox_content_exists = exists(
            LocalWeb.FIREFOX_LOGO, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            firefox_content_exists
        ), "Firefox content before scrolling is on the page."
        click(firefox_test_site_tab_pattern.target_offset(0, 100))
        repeat_key_down(5)

        firefox_tab_scrolled = exists(
            firefox_tab_scrolled_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        iris_tab_logo_location = find(iris_tab_logo_pattern)
        proper_hamburger_menu_region = Region(
            0, iris_tab_logo_location.y, width=Screen.SCREEN_WIDTH, height=200
        )
        ...
        firefox_tab_scrolled_content_exists = exists(
            firefox_tab_scrolled_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        focus_tab_scrolled_content_exists = exists(
            focus_tab_scrolled_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
