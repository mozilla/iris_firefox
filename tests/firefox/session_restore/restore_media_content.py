...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        first_label_exists = exists(
            first_label_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        ...
        blocked_media_icon_exists = exists(
            blocked_media_icon_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert blocked_media_icon_exists, "Blocked media tab opened"

        second_label_exists = exists(
            second_label_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        new_tab()
        navigate(
            "https://videos.cdn.mozilla.net/uploads/Web_Developer_Tools_in_Firefox_Aurora_10.webm"
        )

        speaker_icon_active_exists = exists(
            speaker_icon_active_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        blocked_media_tab_pinned_exists = exists(
            double_icons, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        last_tab_restored = exists(
            web_developer_tools_tab_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        blocked_media_icon_exists = exists(
            double_icons, FirefoxSettings.FIREFOX_TIMEOUT
        )
        no_speaker_tabs = exists(speaker_icon_active_pattern)
        restore_session_check_result = (
            last_tab_restored and blocked_media_icon_exists and (not no_speaker_tabs)
        )
        assert (
            restore_session_check_result
        ), "Tabs are loaded and media blocked for all tabs"
