...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        tls_certificate_name_highlighted_pattern = Pattern(
            "tls_certificate_name_highlighted.png"
        )
        ...
        assert exists(
            digicert_logo_pattern, Settings.DEFAULT_SITE_LOAD_TIMEOUT
        ), "DigiCert site is successfully opened"

        open_find()
        paste("DigiCert Assured ID TLS CA")
        assert exists(
            tls_certificate_name_highlighted_pattern
        ), "The TLS Certificate is available to download."

        tls_certificate_location = find(tls_certificate_name_highlighted_pattern)
        tls_certificate_region = Region(
            tls_certificate_location.x,
            tls_certificate_location.y,
        ...
        paste("Certificates")
        assert exists(
            view_certificates_button_pattern
        ), "View Certificates button displayed."

        click(view_certificates_button_pattern)
        assert exists(
            certificate_manager_window_title_pattern
        ), "View Certificates window displayed."

        certificate_manager_window_location = find(
            certificate_manager_window_title_pattern
        ).below(200)
        Mouse().move(certificate_manager_window_location)

        tls_certificate_found_grey = scroll_until_pattern_found(
            tls_certificate_name_pattern,
            scroll,
            (-mouse_wheel_steps,),
            50,
        ...
        if tls_certificate_found_grey:
            ...
            tls_certificate_found_white = scroll_until_pattern_found(
                tls_certificate_name_pattern_1,
                scroll,
                (mouse_wheel_steps,),
                50,
                timeout=FirefoxSettings.TINY_FIREFOX_TIMEOUT // 2,
        ...
        assert exists(
            cdn77_logo_pattern, Settings.DEFAULT_HEAVY_SITE_LOAD_TIMEOUT
        ), "CDN77 page is successfully downloaded."
        ...
        assert exists(
            cloudflare_support_page_pattern, Settings.DEFAULT_HEAVY_SITE_LOAD_TIMEOUT
        ), "TLS client certificate authentication mechanism will not be broken. No errors occur."
