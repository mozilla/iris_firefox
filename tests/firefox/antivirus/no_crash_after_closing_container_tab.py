...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        addons_page_opened = exists(
            addons_logo_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert addons_page_opened, "Firefox Add-ons page opened"

        find_addons_field_displayed = exists(
            find_addons_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        multi_account_suggest = exists(
            multi_account_logo_small_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        multi_account_page_opened = exists(
            multi_account_logo_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert multi_account_page_opened, "Firefox Multi-Account Containers page opened"

        add_to_firefox_button_displayed = exists(
            add_to_firefox_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert add_to_firefox_button_displayed, "Add to Firefox button displayed"

        click(add_to_firefox_button_pattern)

        add_addon_popup_displayed = exists(
            add_addon_button_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert (
            add_addon_popup_displayed
        ), "Add Firefox Multi-Account Containers? pop-up displayed"

        click(add_addon_button_pattern)

        addon_has_been_added = exists(
            ok_addon_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            addon_has_been_added
        ), "Firefox Multi-Account Containers has been added to Firefox"

        click(ok_addon_button_pattern)

        navigate("about:newtab")

        youtube_top_sites_logo_exists = exists(
            youtube_top_sites_logo_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert youtube_top_sites_logo_exists, "Youtube top sites logo exists"

        right_click(youtube_top_sites_logo_pattern)

        youtube_container_option_exists = exists(
            container_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert youtube_container_option_exists, "Container option exists"

        click(container_option_pattern)

        youtube_personal_option_exists = exists(
            personal_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert youtube_personal_option_exists, "Container option exists"

        click(personal_option_pattern)

        twitter_top_sites_logo_exists = exists(
            twitter_top_sites_logo_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert twitter_top_sites_logo_exists, "Twitter top sites logo exists"

        right_click(twitter_top_sites_logo_pattern)

        twitter_container_option_exists = exists(
            container_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        twitter_personal_option_exists = exists(
            personal_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        youtube_opened_in_container_tab = exists(
            personal_label_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            youtube_opened_in_container_tab
        ), "Youtube site is opened in container tab"
        ...
        twitter_opened_in_container_tab = exists(
            personal_label_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            twitter_opened_in_container_tab
        ), "Twitter site is opened in container tab"

        close_tab()

        try:
            twitter_tab_closed = wait_vanish(
                twitter_logo_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
        ...
        no_crashes_occurred = exists(
            yotube_logo_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert (
            no_crashes_occurred
        ), "No crash or blue screen after closing a container tab."
