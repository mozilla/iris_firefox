...
class Test(FirefoxTest):
    ...
    def test_run(self, firefox):
        ...
        last_page_contents_rotated_pattern = Pattern(
            "last_page_contents_pres_mode_rotated.png"
        )
        rotate_counterclockwise_option_pattern = Pattern(
            "rotate_counterclockwise_option.png"
        )
        first_page_contents_presentation_mode = Pattern(
            "first_page_contents_pres_mode.png"
        )
        ...
        document_opened = exists(
            first_page_document_contents_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        ...
        assert (
            presentation_mode_button_available
        ), "'Presentation mode' button available"
        ...
        try:
            full_screen_popup_vanished = wait_vanish(
                exit_fullscreen_button_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
            )
            assert (
                full_screen_popup_vanished
            ), "'Full screen' popup successfully vanished"
        except FindError:
            raise FindError("'Full screen' popup did not vanish")

        if OSHelper.is_windows():
            scrolling_works = scroll_until_pattern_found(
                introduction_chapter_headline_pattern, scroll, (-100,)
            )
        else:
            scrolling_works = scroll_until_pattern_found(
                introduction_chapter_headline_pattern, scroll, (-1,)
            )
        ...
        go_to_last_page_option_available = exists(
            go_to_first_page_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            go_to_last_page_option_available
        ...
        last_page_opened = exists(
            last_page_document_contents_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert last_page_opened, "'Go to last page' option works as expected"

        right_click(Location(Screen.SCREEN_WIDTH // 2, Screen.SCREEN_HEIGHT // 2))

        rotate_clockwise_option_available = exists(
            rotate_clockwise_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            rotate_clockwise_option_available
        ...
        page_contents_rotated = exists(
            last_page_contents_rotated_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert page_contents_rotated, "'Rotate clockwise' option works as expected"

        right_click(Location(Screen.SCREEN_WIDTH // 2, Screen.SCREEN_HEIGHT // 2))

        rotate_counterclockwise_option_available = exists(
            rotate_counterclockwise_option_pattern,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
        )
        assert (
            rotate_counterclockwise_option_available
        ...
        ), "'Rotate Counterclockwise' option works as expected"

        right_click(Location(Screen.SCREEN_WIDTH // 2, Screen.SCREEN_HEIGHT // 2))

        go_to_first_page_option_available = exists(
            go_to_first_page_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            go_to_first_page_option_available
        ...
        view_page_info_option_available = exists(
            view_page_info_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            view_page_info_option_available
        ), "'View page info' option available in context menu after right-click at the document area"

        click(view_page_info_pattern)

        page_info_opened = exists(
            page_info_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        save_page_as_option_available = exists(
            save_page_as_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            save_page_as_option_available
        ), "'Save page as...' option available in context menu after right-click at the document area"

        click(save_page_as_option_pattern)

        dialogue_window_opened = exists(
            dialogue_window_cancel_button, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert dialogue_window_opened, "'Save page as...' option works as expected"

        click(dialogue_window_cancel_button)

        try:
            save_as_dialogue_closed = wait_vanish(
                dialogue_window_cancel_button, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
            )
        ...
        inspect_element_option_available = exists(
            inspect_element_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            inspect_element_option_available
        ...
        ), "'Inspect element...' option works as expected"

        close_devtools_button_available = exists(close_devtools_button_pattern)
        assert (
            close_devtools_button_available
        ), "'Close' button available in devtools panel"

        click(close_devtools_button_pattern)

        try:
            dev_console_closed = wait_vanish(
                dev_console_inspector_button_pattern,
                FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
            )
        ...
        exit_full_screen_option_available = exists(
            exit_full_screen_option_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            exit_full_screen_option_available
        ...
        exit_fullscreen_option_works = exists(
            presentation_button_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            exit_fullscreen_option_works
        ), "'Exit Full Screen' option works as expected"

        click(presentation_button_pattern)

        assert exists(exit_fullscreen_button_pattern) and not exists(
            presentation_button_pattern
