...
class Test(FirefoxTest):
    @pytest.mark.details(
        ...
        test_suite_id="1826",
        locales=["en-US"],
        blocked_by={"id": "4387", "platform": OSPlatform.ALL}
    )
    def run(self, firefox):
        ...
        tracking_content_detected_message_pattern = Pattern("tracking_protection_is_off.png")
        tracking_attempts_blocked_message_pattern = Pattern("tracking_protection_is_on.png").similar(0.7)
        ...
        assert private_browsing_window_loaded, "A new private window is successfully opened"
        ...
        assert tracking_protection_shield_displayed, "The tracking protection shield is displayed"

        restore_firefox_focus()
        cnn_blocked_content_displayed = scroll_until_pattern_found(
            cnn_blocked_content_pattern, type, (Key.PAGE_DOWN,), 10, timeout=FirefoxSettings.TINY_FIREFOX_TIMEOUT / 3
        )
        ...
        assert disable_blocking_button_displayed, "The 'Site information' panel is displayed"

        trackers_button_available = exists(trackers_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT)
        assert trackers_button_available, "'Trackers' button is displayed" " on the 'Site information' panel"
        if OSHelper.is_mac():
        ...
        cnn_blocked_content_displayed = scroll_until_pattern_found(
            cnn_blocked_content_pattern, type, (Key.PAGE_DOWN,), timeout=FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        assert site_information_panel_displayed, "The 'Site information' panel is displayed"
        ...
        list_of_active_trackers_displayed = not exists(trackers_icon_pattern, FirefoxSettings.FIREFOX_TIMEOUT)
        assert list_of_active_trackers_displayed, "A list of active trackers is successfully displayed."

        type(Key.ESC)

        tracking_protection_shield_deactivated_displayed = exists(
            tracking_protection_shield_deactivated_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert tracking_protection_shield_deactivated_displayed, "The tracking protection shield is displayed"
        ...
        assert tracking_protection_shield_displayed, "The tracking protection shield is displayed as active"
