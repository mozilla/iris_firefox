...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        assert browser_console_opened is True, (
            "Browser Console is successfully opened. "
        ...
        try:
            private_mozilla_closed = wait_vanish(
                private_window_inactive_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
            assert private_mozilla_closed is True, (
                "Firefox restarted. " "Private browsing session is not restored."
            )
        except FindError:
            raise FindError("Private browser windows was not closed.")

        browser_console_active = exists(
            browser_console_title_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        assert (
            browser_console_active is True
        ), "Browser Console is active. Closing Browser Console."

        close_tab()

        try:
            browser_console_closed = wait_vanish(
                browser_console_title_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
            assert (
                browser_console_closed is True
