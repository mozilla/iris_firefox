...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        do_not_track_selected_pattern = Pattern(
            "do_not_track_option_always_selected_radio.png"
        )
        tracker_website_content_pattern = Pattern("tracker_website_content.png")
        do_not_track_signal_displayed_pattern = Pattern(
            "do_not_track_signal_displayed.png"
        )

        navigate("about:preferences#privacy")

        privacy_preferences_page_displayed = exists(
            preferences_privacy_find_field_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            privacy_preferences_page_displayed is True
        ...
        send_track_data_found = scroll_until_pattern_found(
            send_track_data_pattern, scroll_down, (25,), 20, 1
        )
        assert send_track_data_found is True, "Send websites option found"

        send_track_data_pattern_width, send_track_data_pattern_height = (
            send_track_data_pattern.get_size()
        )

        send_websites_option_position = find(send_track_data_pattern)
        send_websites_option_region = Region(
            send_websites_option_position.x - 100,
            send_websites_option_position.y,
            send_track_data_pattern_width + 100,
            send_track_data_pattern_height + 100,
        )

        send_websites_option_unchecked = exists(
            do_not_track_unselected_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            send_websites_option_region,
        )
        assert (
            send_websites_option_unchecked is True
        ), 'Do not track "Always" option is displayed unchecked'

        click(do_not_track_unselected_pattern, region=send_websites_option_region)

        do_not_track_selected = exists(
            do_not_track_selected_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            send_websites_option_region,
        )
        ...
        website_loaded = exists(
            tracker_website_content_pattern, FirefoxSettings.SITE_LOAD_TIMEOUT
        )
        ...
        assert (
            do_not_track_signal_displayed is True
