...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        private_content_blocking_warning_pattern = Pattern(
            "private_window_content_blocking_warning.png"
        )
        ...
        assert (
            navigated_to_preferences is True
        ), "The about:preferences#privacy page is successfully displayed."

        move(Location(Screen.SCREEN_WIDTH // 2, Screen.SCREEN_HEIGHT // 2))

        custom_radio_found = scroll_until_pattern_found(
            custom_privacy_radio_pattern, scroll_down, (25,), 20, 1
        )
        assert (
            custom_radio_found is True
        ...
        assert (
            content_blocking_trackers_unchecked is True
        ...
        assert (
            content_blocking_cookies_unchecked is True
        ), "The cookies checkbox is unchecked successfully."
        ...
        cnn_logo_exists = exists(
            cnn_logo_pattern, FirefoxSettings.HEAVY_SITE_LOAD_TIMEOUT
        )
        assert cnn_logo_exists is True, "The website is successfully displayed."

        tracking_protection_shield_common_window = exists(
            tracking_protection_shield_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert tracking_protection_shield_common_window is False, (
        ...
        private_window_opened = exists(
            private_browsing_tab_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert private_window_opened is True, "Private browsing window opened"

        content_blocking_private_displayed = exists(
            private_content_blocking_warning_pattern,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
        )
        assert content_blocking_private_displayed is False, (
            '"Some websites use trackers'
        ...
        tracking_protection_shield_private_window = exists(
            tracking_protection_shield_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert tracking_protection_shield_private_window is False, (
            "The Content blocking shield" " is not displayed near the address bar"
        )

        info_button_located = exists(
            info_button_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            info_button_located is True
        ), 'The "i button" (Info button) is located near the address bar'

        click(info_button_pattern)

        open_trackers_list_button_located = exists(
            trackers_button_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            open_trackers_list_button_located is True
        ...
        assert (
            trackers_popup_displayed is True
        ), "Trackers popup window is displayed on screen"

        list_of_active_trackers_displayed = exists(
            trackers_icon_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            list_of_active_trackers_displayed is True
        ), "A list of active trackers is successfully displayed."

        trackers_blocked = exists(
            blocked_tracker_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert trackers_blocked is False, "The trackers are not blocked."

        to_block_set_to_strict_located = exists(
            to_block_set_to_strict_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert to_block_set_to_strict_located is True, (
