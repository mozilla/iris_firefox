...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        private_window_opened = exists(
            PrivateWindow.private_window_pattern.similar(0.6)
        )
        ...
        window_closed = exists(
            LocalWeb.SOAP_WIKI_SOAP_LABEL, FirefoxSettings.TINY_FIREFOX_TIMEOUT
        )
        assert window_closed is False, "The window is closed"

        firefox_icon_dock_exists = exists(
            Docker.FIREFOX_DOCKER_ICON, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
        ...
        click(
            Docker.FIREFOX_DOCKER_ICON,
            FirefoxSettings.TINY_FIREFOX_TIMEOUT,
            region=dock_region,
        )

        top_sites_item_exists = exists(
            top_sites_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            top_sites_item_exists is True
        ), "New window with new tab opened successfully."
        ...
        wiki_soap_history_icon_exists = exists(
            wiki_soap_history_icon_pattern, FirefoxSettings.TINY_FIREFOX_TIMEOUT
        )
        assert (
            wiki_soap_history_icon_exists is False
        ), "The Recent History section is empty."
