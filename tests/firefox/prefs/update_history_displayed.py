...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        choose_language_button = scroll_until_pattern_found(
            show_update_history_button_pattern,
            Mouse().scroll,
            (None, -scroll_height),
            140,
            1,
        ...
        update_history_popup_title = exists(
            update_history_popup_title_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert update_history_popup_title, 'The "Update History" subdialog appears.'

        message_region = Region(
            0,
            int(Screen.SCREEN_HEIGHT / 3),
            Screen.SCREEN_WIDTH,
            int(Screen.SCREEN_HEIGHT / 3),
        )

        no_updates_installed = exists("No updates installed yet", region=message_region)
        assert no_updates_installed, (
            '"No updates installed yet" displayed. A list with updates that were installed '
