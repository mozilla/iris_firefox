...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        if OSHelper.is_linux():
            use_autoscrolling_unchecked_exists = scroll_until_pattern_found(
                use_autoscrolling_unchecked_pattern,
                Mouse().scroll,
                (0, -Screen.SCREEN_HEIGHT),
                20,
                FirefoxSettings.TINY_FIREFOX_TIMEOUT / 2,
            )
            assert (
                use_autoscrolling_unchecked_exists
            ), "The option is not checked by default"

            click(use_autoscrolling_unchecked_pattern)

            use_autoscrolling_checked_exists = exists(
                use_autoscrolling_checked_pattern, FirefoxSettings.FIREFOX_TIMEOUT
            )
            assert use_autoscrolling_checked_exists, "The option is checked"

        else:
            use_autoscrolling_checked_exists = scroll_until_pattern_found(
                use_autoscrolling_checked_pattern,
                Mouse().scroll,
                (0, -Screen.SCREEN_HEIGHT),
                20,
                FirefoxSettings.TINY_FIREFOX_TIMEOUT / 2,
        ...
        autoscrolling_enabled_exists = exists(
            autoscrolling_enabled_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        ...
        use_autoscrolling_unchecked_exists = exists(
            use_autoscrolling_unchecked_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            use_autoscrolling_unchecked_exists
        ), "The option is successfully unchecked"
