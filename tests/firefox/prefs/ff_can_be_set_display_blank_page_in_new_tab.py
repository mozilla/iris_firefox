...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        preferences_page_opened = exists(
            new_tab_preferences_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
        ...
        new_tab_preferences_width, new_tab_preferences_height = (
            new_tab_preferences_pattern.get_size()
        )
        new_tab_section_region = Region(
            new_tab_preferences_location.x,
            new_tab_preferences_location.y,
            new_tab_preferences_width * 10,
            new_tab_preferences_height * 2,
        )

        new_tab_option_displayed = exists(
            default_setting_home_pattern,
            FirefoxSettings.FIREFOX_TIMEOUT,
            new_tab_section_region,
        )
        assert (
            new_tab_option_displayed
        ), 'The options for "New Tabs" section are displayed'
        ...
        assert (
            home_page_loaded is False
        ), "The Firefox Home page is successfully loaded."
