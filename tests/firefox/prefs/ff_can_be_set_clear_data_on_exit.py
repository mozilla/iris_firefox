...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        clear_form_search_pattern = Pattern("clear_form_search.png")
        ...
        always_private_region = Region(
            always_private_location.x - box_width * 2,
            always_private_location.y,
            always_private_width + box_width * 2,
            always_private_height,
        )

        always_private_unchecked = exists(
            prefs_unchecked_box_pattern, FirefoxSettings.FIREFOX_TIMEOUT, region=always_private_region
        )
        ...
        remember_browsing_history_checked = exists(
            prefs_checked_box_pattern, FirefoxSettings.FIREFOX_TIMEOUT, region=remember_browsing_history_region
        )
        assert (
            remember_browsing_history_checked
        ), '"Always use private browsing mode" is off'

        remember_search_history_displayed = exists(remember_search_history_pattern, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        remember_search_history_checked = exists(
            prefs_checked_box_pattern, FirefoxSettings.FIREFOX_TIMEOUT, region=remember_search_history_region
        )
        assert (
        ...
        clear_history_unchecked = exists(
            prefs_unchecked_box_pattern, FirefoxSettings.FIREFOX_TIMEOUT, region=clear_history_closing_region
        )
        ...
        clear_history_settings_opened = exists(clear_browsing_download_pattern, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        settings_still_opened = exists(clear_form_search_pattern, FirefoxSettings.FIREFOX_TIMEOUT)
        assert settings_still_opened, "Settings are still opened and \"Clear browsing and download\" is checked"

        click(clear_form_search_pattern)

        clear_form_search_location = find(clear_form_search_pattern)
        clear_form_search_width, clear_form_search_height = clear_form_search_pattern.get_size()
        clear_form_search_region = Region(clear_form_search_location.x,
                                          clear_form_search_location.y,
                                          clear_form_search_width,
                                          clear_form_search_height)
        clear_form_search_unchecked = exists(
            AboutPreferences.UNCHECKED_BOX, region=clear_form_search_region
        )
        assert (
        ...
        pdf_downloaded = exists(NavBar.DOWNLOADS_BUTTON, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        library_menu_opened = exists(LibraryMenu.HISTORY_BUTTON, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        browsing_history_purged = exists(LocalWeb.FIREFOX_BOOKMARK, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        library_menu_still_opened = exists(LibraryMenu.DOWNLOADS, FirefoxSettings.FIREFOX_TIMEOUT)
        ...
        if OSHelper.is_mac():
            ...
            open_library()
            library_window_opened = exists(Library.DOWNLOADS, FirefoxSettings.FIREFOX_TIMEOUT)
            ...
            downloads_menu_opened = exists(DownloadManager.SHOW_ALL_DOWNLOADS, FirefoxSettings.FIREFOX_TIMEOUT)
