...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        cursor_keys_to_navigate_checkbox_pattern = Pattern("always use the cursor keys to navigate.png").similar(0.6)
        ...
        assert general_prefs_section_opened, "'General' section in 'Preferences' couldn't be opened"

        paste("cursor keys to navigate")

        cursor_keys_to_navigate_checkbox_available = exists(cursor_keys_to_navigate_checkbox_pattern)
        assert (
            cursor_keys_to_navigate_checkbox_available
        ), "'Always use the cursor keys to navigate within pages' checkbox is not available " "in 'General' preferences section"

        cursor_keys_to_navigate_location = find(cursor_keys_to_navigate_checkbox_pattern)
        ...
        cursor_keys_to_navigate_checked = exists(AboutPreferences.CHECKED_BOX, region=cursor_keys_to_navigate_region)
        assert (
        ...
        soap_local_text_location = find(soap_wiki_local_text_without_cursor_reference)

        soap_local_text_width, soap_local_text_height = soap_wiki_local_text_without_cursor_reference.get_size()
        ...
        cursor_keys_appears_on_click = exists(
            soap_wiki_local_text_with_cursor,
        ...
        cursor_keys_should_not_appears_on_click = exists(
            soap_wiki_local_text_without_cursor,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
            region=soap_local_text_location_region,
        )
        assert (
            cursor_keys_should_not_appears_on_click
        ), "The cursor keys (the '|' symbol) is still appearing " "in the referenced text"
        ...
        cursor_keys_does_not_appears_on_click = exists(
            soap_wiki_local_text_without_cursor,
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT,
            region=soap_local_text_location_region,
        )
