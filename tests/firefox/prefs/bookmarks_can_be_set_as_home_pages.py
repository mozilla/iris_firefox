...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        preferences_page_opened = exists(
            homepage_preferences_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
        ...
        homepage_preferences_width, homepage_preferences_height = (
            homepage_preferences_pattern.get_size()
        )
        homepage_section_region = Region(
            homepage_preferences_location.x,
            homepage_preferences_location.y,
            homepage_preferences_width * 3,
            int(homepage_preferences_height * 1.5),
        )

        home_option_displayed = exists(
            default_setting_home_pattern,
        ...
        custom_option_displayed = exists(
            custom_url_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            custom_option_displayed
        ), "The 'Custom' option for \"Home\" section is displayed."

        click(custom_url_option_pattern)

        bookmark_option_displayed = exists(
            url_field_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        bookmark_option_displayed = exists(
            use_bookmark_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert bookmark_option_displayed, "The 'Use bookmark...' button is displayed."

        custom_option_displayed = exists(
            use_current_page_option_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        getting_started_bookmark_displayed = exists(
            getting_started_bookmark_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            getting_started_bookmark_displayed
        ), "Thr Getting Started bookmark displayed"
        ...
        assert (
            "https://www.mozilla.org/en-US/firefox/central/" in field_populated
        ...
        home_page_displayed = exists(
            getting_started_tab_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
