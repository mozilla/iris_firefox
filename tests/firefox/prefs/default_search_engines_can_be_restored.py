...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        google_one_click_search = exists(
            google_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert (
            google_one_click_search
        ), 'Google is in the "One-click Search Engines" list'

        bing_one_click_search = exists(
            bing_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert bing_one_click_search, 'Bing is in the "One-click Search Engines" list'

        amazon_one_click_search = exists(
            amazon_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert (
            amazon_one_click_search
        ), 'Amazon is in the "One-click Search Engines" list'

        duck_one_click_search = exists(
            duck_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert (
            duck_one_click_search
        ), 'DuckDuckGo is in the "One-click Search Engines" list'

        ebay_one_click_search = exists(
            ebay_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert ebay_one_click_search, 'Ebay is in the "One-click Search Engines" list'

        twitter_one_click_search = exists(
            twitter_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert (
            twitter_one_click_search
        ), 'Twitter is in the "One-click Search Engines" list'

        wiki_one_click_search = exists(
            wiki_one_click_search_pattern, region=Screen.LEFT_HALF
        )
        assert (
            wiki_one_click_search
        ...
        )  # wait for load of active button Remove

        bottom_region = Screen.BOTTOM_HALF

        remove_available = exists(
            remove_search_system_pattern, FirefoxSettings.FIREFOX_TIMEOUT, bottom_region
        )
        ...
        time.sleep(
            FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )  # wait until search engine actually removes
        ...
        assert bing_one_click_search, (
            "At the bottom of the dropdown, the deleted search engine is not listed "
            "among the others."
        )
