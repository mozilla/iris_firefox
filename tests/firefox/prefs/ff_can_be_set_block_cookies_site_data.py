...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        standard_security_level_exists = exists(
            standard_security_level_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        time.sleep(
            Settings.DEFAULT_UI_DELAY
        )  # to prevent clicking on wrong location due to fast execution
        click(manage_data_button_pattern)

        empty_dialog_exists = not exists(
            empty_sites_pattern, FirefoxSettings.SHORT_FIREFOX_TIMEOUT
        )
        assert (
            empty_dialog_exists
        ), "The dialog is opened and it is contains only addons site"
        ...
        block_button_exists = exists(
            block_button_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        ...
        reddit_label_not_in_list = not exists(
            reddit_label_in_list_pattern, FirefoxSettings.FIREFOX_TIMEOUT
        )
        assert (
            reddit_label_not_in_list
        ), "https://www.reddit.com/ is NOT displayed on that list"
