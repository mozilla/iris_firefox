...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        restore_previous_session_checkbox_pattern = Pattern(
            "restore_previous_session_checkbox.png"
        )

        navigate("about:preferences#general")

        restore_previous_session_checkbox_displayed = exists(
            restore_previous_session_checkbox_pattern, 10
        )
        assert (
            restore_previous_session_checkbox_displayed is True
        ...
        history_restored_cnn = exists(
            LocalWeb.CNN_LOGO.similar(0.6), region=history_sidebar_region
        )
        ...
        history_restored_youtube = exists(
            youtube_logo_pattern, region=history_sidebar_region
        )
        assert history_restored_youtube is True, "The Youtube site is added to history"

        history_restored_twitter = exists(
            twitter_logo_pattern, region=history_sidebar_region
        )
        ...
        tab_restored_youtube = exists(
            youtube_logo_unactive_tab_pattern, region=tabs_region
        )
        assert tab_restored_youtube is True, "The Youtube tab is restored"

        tab_restored_twitter = exists(
            twitter_logo_unactive_tab_pattern, region=tabs_region
        )
