...
class Test(FirefoxTest):
    ...
    def run(self, firefox):
        ...
        certificate_manager_window_title_pattern = Pattern("certificate_manager_window_title.png").similar(0.7)
        ...
        tls_certificate_region = Region(
            tls_certificate_location.x, tls_certificate_location.y, Screen.SCREEN_WIDTH / 3, Screen.SCREEN_HEIGHT / 10
        )
        ...
        assert exists("Help", FirefoxSettings.FIREFOX_TIMEOUT, region=cdn_region), "CDN77 Support button is displayed."
        ...
        assert exists(
            cloudflare_logo_pattern, Settings.DEFAULT_HEAVY_SITE_LOAD_TIMEOUT, tabs_region
        ), "Cloudflare page is successfully downloaded."

        scroll_until_pattern_found(
            cloudflare_support_button_pattern.similar(0.7), Mouse().scroll, (None, -mouse_wheel_steps)
        )
