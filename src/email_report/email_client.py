...
class EmailClient:
    ...
    def send_email_report(self, target: str, test_status: str, repo_details: str):
        ...
        if isinstance(repo_details, dict):
            body_message = MIMEText('''Repo_details:
 Branch_name: {}
 Branch_head: {}

 Test_Run_Details: {}
 Note: To see the complete run output, please check the attachment.'''.format(repo_details.get('iris_branch'), 
    repo_details.get('iris_branch_head'), test_status))
        else:
