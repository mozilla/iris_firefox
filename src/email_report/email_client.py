...
class EmailClient:
    ...
    def create_email_subject(target):
        email_info = '[{}][{}]Iris Test Report {}' .format(
            "{} {}".format(target.target_name, target.values['fx_version']) if target.target_name == 'Firefox'
...
Note: To see the complete run output, please check the attachment.'''.format(
    repo_details.get('iris_branch'), repo_details.get('iris_branch_head'), test_status))
        else:
